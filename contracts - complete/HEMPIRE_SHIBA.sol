...
contract HEMPIRE_SHIBA {
    // real
    // address shib = 0x2859e4544c4bb03966803b044a93563bd2d0dd4d;
    // test
    address shib = 0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee;
    ...
    uint256 public MaxLimitInvest = 2000;
    bool public isSetMaxLimit = false;
    ...
    function collect() public {
        ...
        ERC20(shib).transfer(ceoAddress, devfee);
        ERC20(shib).transfer(ceoAddress2, ceofee);
        ERC20(shib).transfer(ceoAddress3, ceofee1);
        ERC20(shib).transfer(ceoAddress4, ceofee2);
        ERC20(shib).transfer(address(msg.sender), SafeMath.sub(eggValue,fee));
    }
    function hire(address ref, uint256 amount) public {
        require(initialized);
        if (totalInvestment[msg.sender] > 0) {
            require(totalInvestment[msg.sender] <= MaxLimitInvest * (10**18), "Max limit is 2000 BUSD");
        } else {
            require(amount <= MaxLimitInvest * (10**18), "Max limit is 2000 BUSD");
        }
        // require(amount >= 25 * (10**18), "Min limit is 25 BUSD");
        // ERC20(shib).approve(address(msg.sender), amount);
        ...
        if (totalInvestment[msg.sender] > 0) {
            totalInvestment[msg.sender] += amount;
        } else {
            totalInvestment[msg.sender] = amount;
        }
        compound(ref);
    ...
    function setMaxLimitAmount(uint256 _limit) public {
        require(msg.sender == ceoAddress, "Ownable: caller is not the owner");
        require(_limit > 0, "limit should be more that zero");
        MaxLimitInvest = _limit;
    }

    function setMaxLimit() public {
        require(msg.sender == ceoAddress, "Ownable: caller is not the owner");
        isSetMaxLimit = !isSetMaxLimit;
    }
}
